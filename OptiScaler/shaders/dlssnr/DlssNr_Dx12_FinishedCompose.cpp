                                            ...
                                            DXGI_COLOR_SPACE_TYPE colorSpace, bool gameFrameHandoff) -> bool
{
    ...
    for (auto& slot : late.slots)
    {
                                        ...
                                        slot.ready))
        {
            ...
            {
                LOG_INFO("DLSS-NR finished picture: skipping unfinished cross-queue input to avoid a present/render "
                         "fence cycle");
    ...
    if (slot.residualOnly)
    {
        ...
        if (slot.encoded && Config::Instance()->DlssNrApplyModel.value_or_default())
        {
            ...
            apply.MvScaleX = std::clamp(Config::Instance()->DlssNrShadowFloor.value_or_default(), 0.0f,
                                        1.0f); // finished shader alias
